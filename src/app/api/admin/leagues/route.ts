...
export async function GET(request: NextRequest) {
  try {
    ...
    const url = `${API_BASE_URL}/admin/leagues${queryString ? `?${queryString}` : ''}`;
    ...
    if (!response.ok) {
      ...
      console.error('Leagues list proxy error:', response.status, errorText);
    ...
    console.error('Leagues list proxy error:', error);
    return NextResponse.json(
