...
export function SidebarArticleItem({
  ...
  const content = (
    <div className="flex gap-3 items-start">
      <div className="flex-shrink-0 w-16 h-16 rounded-xl overflow-hidden bg-gradient-to-br from-muted to-muted/80 shadow-sm">
        {article.imageUrl && !article.imageUrl.includes('default-transfer') ? (
          <Image
  ...
  const linkClass = cardStyle
    ? "block group rounded-lg p-3 hover:bg-muted/50 transition-colors duration-200"
    : "block group hover:bg-muted/50 rounded-lg p-3 -m-3 transition-colors duration-200";
